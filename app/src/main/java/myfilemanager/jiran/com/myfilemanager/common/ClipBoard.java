...
public final class ClipBoard {
     ...
     * Files in clip board
     */
    private static ArrayList<FileItem> clipBoard;
    ...
    public static synchronized void cutCopy(ArrayList<FileItem> files) {
        synchronized (LOCK) {
            if (!isLocked) {
                isMove = false;
                clipBoard = files;
    ...
    public static synchronized void cutMove(ArrayList<FileItem> files) {
        synchronized (LOCK) {
            if (!isLocked) {
                isMove = true;
                clipBoard = files;
    ...
    public static ArrayList<FileItem> getClipBoardContents() {
        synchronized (LOCK) {
            return clipBoard;
        }
    }
